        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-8">
            <nav className="hidden md:flex items-center space-x-8">
              ...
              <Link className="text-gray-700 hover:text-primary transition-colors" to="/pacotes">
                Roteiros
              </Link>
            </nav>
            <div className="flex items-center space-x-4">
              ...
              <Link to="/admin" className="text-sm text-gray-600 hover:text-primary">
                Admin
              </Link>
            </div>
          </div>
...
export function Footer() {
  return (
    <footer className="mt-20 border-t bg-footer" id="contato">
      <div className="max-w-6xl mx-auto px-4 py-10 grid md:grid-cols-4 gap-8 text-sm">
        ...
        <div>
          ...
          <div className="flex gap-3 mt-3 text-muted-foreground">
            <a
              href="https://www.instagram.com/bellarendaeviagens/"
              target="_blank"
      ...
      <div className="border-t text-center py-4 text-xs text-foreground/80">
        © {new Date().getFullYear()} BELLA RENDA & VIAGENS. Todos os direitos reservados.
      </div>
    </footer>
  );
}
    ...
    return (
      <Card className="overflow-hidden group">
        <div className="relative">
          ...
          <Badge className="absolute top-2 left-2 capitalize bg-primary hover:bg-primary/90 text-white">
            {pkg.type}
          </Badge>
        </div>
        <CardHeader>
    ...
    <Card className="overflow-hidden group relative border-none shadow-md">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-white/95 backdrop-blur-sm p-3 text-center rounded-t-md">
        ...
        <Link to={`/pacotes/${pkg.slug}`}>
          ...
          <Button
            size="sm"
            className="mt-1 bg-[hsl(var(--button-on-primary))] hover:bg-[hsl(var(--button-on-primary)/0.9)] text-white"
          >
            Ver todos
          </Button>
        </Link>
...
function Row({ title, subtitle, filter, primaryColor = false }) {
  ...
  // Criar um ID único para cada carrossel
    ...
    <section className="max-w-6xl mx-auto px-4 mt-12">
      ...
      <div
        className={`p-4 flex items-center justify-between relative ${
          !primaryColor ? 'bg-primary/20 text-primary' : ''
        }`}
        style={headerStyle}
      >
        <div>
      ...
      <div className="p-4 pt-6">
        ...
        <p className="text-center text-xs text-gray-600 mt-4">
          Cada pacote inclui suporte especializado, opções de customização e um ambiente seguro para
          mulheres que decidem dizer "eu vou".
        </p>
      </div>
    </section>
  );
...
export function ContactCTA({ compact = false }) {
  if (compact) {
    return (
    ...
    <section className="max-w-3xl mx-auto px-4 mt-16" id="contato">
      <div className="text-center">
        <h2 className="text-2xl md:text-3xl font-bold">
          Está preparada para dizer <span className="text-[hsl(340,45%,54%)]">sim para você?</span>
        </h2>
        <p className="text-sm text-gray-600 mt-2 max-w-2xl mx-auto">
          Entre em contato e descubra como podemos transformar sua próxima viagem em uma experiência
          inesquecível.
        </p>
      </div>

      <div className="mt-8 shadow-lg p-6 md:p-8 rounded-xl bg-[hsl(340,45%,54%)]">
        <ContactForm />
      </div>
    </section>
  );
...
export function Home() {
  ...
  return (
    <SiteShell>
        ...
        <div className="max-w-6xl mx-auto px-4 py-16 md:py-24 grid md:grid-cols-2 gap-10 items-center relative z-10">
          <div className="text-gray-800 relative z-10">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-6">
              Viajar não é apenas <br /> um destino, é um{' '}
              <span className="text-primary">encontro com você!</span>
            </h1>
      ...
      <div className="relative">
        <div className="absolute inset-0 z-0">
          <img src={icLib3} alt="" className="w-full h-full object-cover opacity-5" />
        </div>
      </div>
      ...
      <section className="max-w-6xl mx-auto px-4 mt-16 relative">
        ...
        <div className="grid md:grid-cols-4 gap-6 relative z-10">
          ...
          {[
            'Momento só seu',
            'Suporte desde a reserva até o retorno',
            'Vivências reais',
            'Flexibilidade',
          ].map((text, idx) => (
            <Card
              key={`benefit-${idx}`}
              className="p-6 text-center bg-primary text-white shadow-lg hover:shadow-xl transition-shadow group"
            >
