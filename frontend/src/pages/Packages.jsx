...
import { loadPackages, saveLead, months as monthList } from "../mock";
      ...
      <section className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mt-6 grid md:grid-cols-4 gap-4">
          <Card className="p-4 md:col-span-1">
            ...
            <ToggleGroup type="single" value={type} onValueChange={(v)=> setType(v||"todos")} className="flex flex-wrap gap-2">
            ...
            <div className="px-1"><Slider value={duration} onValueChange={setDuration} min={1} max={10} step={1} /></div>
            <div className="flex justify-between text-xs text-muted-foreground"><span>{duration[0]}d</span><span>{duration[1]}d</span></div>
            <div className="mt-4 text-sm font-medium">Mês/Temporada</div>
            <Select value={month} onValueChange={setMonth}>
              <SelectTrigger className="mt-2"><SelectValue placeholder="Todas"/></SelectTrigger>
              <SelectContent>
                <SelectItem value="todas">Todas</SelectItem>
                {monthList.map(m => <SelectItem key={m} value={m}>{m}</SelectItem>)}
              </SelectContent>
            </Select>
            <div className="mt-4 text-sm font-medium">Buscar</div>
            <Input value={term} onChange={(e)=> setTerm(e.target.value)} placeholder="Destino, hotel..." className="mt-2"/>
            <Button className="mt-4 w-full" onClick={()=> nav(`/pacotes?tipo=${type}`)}>Aplicar filtros</Button>
          </Card>
          <div className="md:col-span-3">
            ...
            {!filtered.length && (
              <div className="text-center text-muted-foreground py-10">Nenhum pacote encontrado para os filtros selecionados.</div>
            )}
          </div>
        </div>
      </section>
...
export function PackageDetail() {
  ...
  const submit = (e) => {
    e.preventDefault();
    saveLead({ ...lead, pacoteId: pkg.id, pacoteTitulo: pkg.title });
    ...
    <SiteShell>
      <section className="max-w-6xl mx-auto px-4 py-8 grid md:grid-cols-3 gap-8">
        <div className="md:col-span-2">
            ...
            <CarouselContent>
              {(pkg.images||[]).map((src, idx) => (
                <CarouselItem key={idx} className="pr-4">
                  <AspectRatio ratio={16/9}><img src={src} alt={`${pkg.title}-${idx}`} className="w-full h-full object-cover rounded-md"/></AspectRatio>
                </CarouselItem>
              ))}
            </CarouselContent>
            <CarouselPrevious />
          ...
          <div className="text-muted-foreground">{pkg.destination} • {pkg.duration} dias • {pkg.region}</div>
          <div className="mt-4">
            <div className="font-semibold">A partir de R$ {pkg.priceFrom?.toLocaleString("pt-BR")}</div>
            <div className="mt-2 text-sm">Inclui: {(pkg.inclusions||[]).join(", ")}</div>
          </div>
          <p className="mt-4 text-sm md:text-base text-muted-foreground">{pkg.longDescription}</p>
        </div>
        <Card className="p-4 h-fit">
          ...
          <form onSubmit={submit} className="space-y-3">
            <div>
              ...
              <Input required value={lead.nome} onChange={(e)=> setLead({ ...lead, nome: e.target.value })} />
            </div>
            <div>
            ...
            <div>
              ...
              <Input required value={lead.telefone} onChange={(e)=> setLead({ ...lead, telefone: e.target.value })} />
            </div>
            <div>
              <label className="text-sm">Mensagem</label>
              <Textarea value={lead.mensagem} onChange={(e)=> setLead({ ...lead, mensagem: e.target.value })} placeholder="Quero mais informações sobre datas e valores." />
            </div>
